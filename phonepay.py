...
# Safe slider function to handle single-value cases
def safe_slider(label, series):
    unique_vals = series.unique()
    if len(unique_vals) == 1:
        st.write(f"{label}: {unique_vals[0]}")  # Show the value instead of slider
        return unique_vals[0]
    else:
        return st.slider(label, int(series.min()), int(series.max()), int(series.min()))
...
if select == "Home":
    ...
    with col2:
        st.image(Image.open(r"C:\Users\salom\OneDrive\Desktop\proj1.jpg"),width= 600)

    col3,col4= st.columns(2)
    with col3:
        st.image(Image.open(r"C:\Users\salom\OneDrive\Desktop\Alfred\project2.png"),width=600)
    ...
    with tab2:
        ...
        if method_map == "Map Insurance Analysis":
            ...
            col1,col2= st.columns(2)
            with col1:
                quarters_m1 = safe_slider("**Select the Quarter_mi**", df_map_insur_Y["Quarter"])
            ...
            col1,col2= st.columns(2)
            with col1:
                quarters_m2 = st.selectbox("**Select the Quarter_mi**",sorted(df_map_tran_Y["Quarter"].unique()))
    ...
    with tab3:
        ...
        if method_top == "Top Insurance Analysis":
            ...
            col1,col2= st.columns(2)
            with col1:
                quarters_t1 = st.selectbox("**Select the Quarter_ti**", sorted(df_top_insur_Y["Quarter"].unique()))
            ...
            col1,col2= st.columns(2)
            with col1:
                quarters_t2 = st.selectbox("**Select the Quarter_tt**",sorted(df_top_tran_Y["Quarter"].unique()))
